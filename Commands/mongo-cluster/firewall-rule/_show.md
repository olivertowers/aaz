# [Command] _mongo-cluster firewall-rule show_
...
- Gets a firewall rule on a Mongo Cluster resource.
    ```bash
        mongo-cluster firewall-rule show --resource-group TestGroup --mongo-cluster-name myMongoCluster --firewall-rule-name rule1
    ```
